#!/usr/bin/env python3
"""
Fetch new publications from Chelsey Beese's ORCID record and append any
that aren't already in my-pubs.bib.
...
How it works:
  ...
  3. Skip anything whose DOI is already in my-pubs.bib.
  4. Append new entries to my-pubs.bib in the same format as the existing
     ones. Existing entries are never modified or reordered -- this only
     ever adds.
...
ORCID_ID = "0000-0003-0949-7888"  # Chelsey Beese
BIB_PATH = Path(__file__).resolve().parent.parent / "my-pubs.bib"
CONTACT_EMAIL = "beese.c.m@gmail.com"  # used in Crossref's "polite pool" header
USER_AGENT = f"chelsey-beese-website-bot/1.0 (mailto:{CONTACT_EMAIL})"
...
def make_key(work, used_keys):
    # Match the "family_firsttitleword_year" style already used throughout
    # my-pubs.bib (e.g. "beese_smallscale_2023"), so auto-added entries look
    # like the hand-added ones rather than a different citation-key style.
    family = "unknown"
    authors = work.get("author") or []
    if authors:
        family = re.sub(r"[^A-Za-z]", "", authors[0].get("family", "unknown")).lower() or "unknown"

    title = (work.get("title") or [""])[0]
    first_word = re.sub(r"[^A-Za-z0-9]", "", (title.split()[0] if title.split() else "")).lower()

    year = str((work.get("issued", {}).get("date-parts") or [[None]])[0][0] or "nd")
    base = "_".join(p for p in (family, first_word, year) if p)
    key = base
    suffix = ord("a")
